...
= 1.10.76 =
* Fix: Retry SoftOne customer creation with the automatic C-code seed when the tenant rejects generated C-number customer codes.

= 1.10.75 =
* Fix: Generate web customer codes as plain SoftOne C + five digit values while keeping the web-customer marker in CUSEXTRA.

= 1.10.74 =
* Fix: Create SoftOne customers for new checkout orders even when the getCustomers query returns unrelated rows for a code/email filter.
* Fix: Retry temporarily incomplete order exports instead of permanently skipping not-ready WooCommerce orders.

= 1.10.73 =
* Fix: Return no-payment checkout success before slow payment-complete callbacks can stall the browser redirect.

= 1.10.72 =
* Fix: Return the no-payment checkout success redirect immediately after WooCommerce completes the order, avoiding slow late checkout callbacks.

= 1.10.71 =
* Diagnostics: Add checkout shutdown timing and late WooCommerce payment/status breadcrumbs so slow post-order requests can be pinpointed without verbose hook snapshots.
...
= 1.10.68 =
* Fix: Recover duplicate empty-cart checkout AJAX attempts by redirecting to the recent processed order instead of showing a session-expired error.

= 1.10.67 =
* Fix: Run SoftOne exports as a 10-second background job from frontend order requests so WooCommerce checkout stays independent for existing and new customers.

= 1.10.66 =
* Fix: Delay checkout-deferred SoftOne exports by five minutes so WP-Cron cannot run the ERP export inside the same checkout request.

= 1.10.65 =
* Fix: Defer checkout-triggered SoftOne order exports so WooCommerce can return the thank-you redirect before slow ERP calls run.

= 1.10.64 =
* Fix: Correct the controlled checkout customer profile update bypass and add user-meta update diagnostics.

= 1.10.63 =
* Change: Add checkout customer-update hook diagnostics to identify the next callback blocking new-customer order creation.

= 1.10.62 =
* Change: Add a narrow diagnostic bypass for checkout customer profile updates in the controlled SoftOne support email and test coupon flow.

= 1.10.61 =
* Change: Improve the controlled checkout diagnostic email bypass by removing WooCommerce's created-customer email callback even when it was registered on a different mailer instance.

= 1.10.60 =
* Change: Add a narrow test-only bypass for WooCommerce new-account email sending in the controlled SoftOne support email checkout flow.

= 1.10.59 =
* Change: Add a narrow checkout attempt throttle bypass for the controlled SoftOne support email and test coupon flow.

= 1.10.58 =
* Change: Log checkout hook callback lists and created-customer completion markers to identify the next plugin blocking new-customer checkout.
...
= 1.10.55 =
* Fix: Skip SoftOne export attempts for pending or incomplete checkout orders so customer creation waits for a stable WooCommerce order snapshot.

= 1.10.54 =
* Fix: Keep generated web customer codes inside SoftOne's exact C + five digit mask and probe for an unused reserved code before customer creation.

= 1.10.53 =
* Fix: Re-link recreated WooCommerce users to existing SoftOne customers by email before checking generated customer codes.

= 1.10.52 =
* Fix: Reserve C8/C9 customer code ranges for web customers and avoid reusing colliding SoftOne customer codes with different emails.

= 1.10.51 =
* Fix: Generate SoftOne customer codes with the required C-prefix format while keeping the web-customer marker in CUSEXTRA.

= 1.10.50 =
* Fix: Create missing SoftOne customers from the order checkout snapshot before SALDOC export and defer order export retries while customer/TRDR resolution catches up.

= 1.10.49 =
* Fix: Score SoftOne response charset conversions so Greek API errors remain readable, and add diagnostics when SoftOne returns unrecoverable question marks.

= 1.10.48 =
* Fix: Convert legacy Greek SoftOne JSON responses to UTF-8 before decoding so order export errors remain readable in logs.
...
= 1.10.43 =
* Change: Order export logs now record the full setData request structure (service/object/AppID/clientID/data) so SALDOC debugging matches the SoftOne example calls.

= 1.10.44 =
* Change: SALDOC request logging now mirrors the SoftOne sample order envelope (service, clientID, appID, object, data) for 1:1 troubleshooting.
